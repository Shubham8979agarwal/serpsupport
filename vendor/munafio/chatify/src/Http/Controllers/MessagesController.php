    ...
    /*public function getContacts(Request $request)
{
    ...
    if (!$website_url) {
        // Handle missing website_url in session
        return Response::json([
    ...
    $users = Message::join('users', function ($join) {
            ...
            ->join('websites', 'users.id', '=', 'websites.user_id') // Join with the websites table
            ->where(function ($q) {
                $q->where('ch_messages.from_id', Auth::user()->id)
                  ->orWhere('ch_messages.to_id', Auth::user()->id);
            })
            ->where('users.id', '!=', Auth::user()->id)
            ->where('websites.website_url', $website_url) // Filter by website_url
...
public function getContacts(Request $request)
{
    ...
    // Retrieve the authenticated user's ID as a UUID
    $userId = Auth::user()->id;

    // Query to get users who have exchanged messages with the authenticated user
    // and match the website_url via the websites table
    $users = Message::join('users', function ($join) use ($userId) {
                $join->on('ch_messages.from_id', '=', 'users.id')
                     ->orOn('ch_messages.to_id', '=', 'users.id');
            })
            ->join('websites', 'users.id', '=', 'websites.user_id')
